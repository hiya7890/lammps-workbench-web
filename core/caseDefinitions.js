(function initLammpsCaseDefinitions(root, factory) {
  ...
  const CASE_TYPES = [
    ...
    },
    {
      id: "cg_scaffold",
      label: "CG scaffold",
      description: "Representative CG molecule/protocol scaffold for Web-safe file generation and local execution handoff.",
      modes: ["web_safe", "local_gui", "local_runner"],
      fields: [
        { key: "title", label: "Title", type: "text", default: "CG scaffold", required: true, validation: { required: true } },
        { key: "units", label: "Units", type: "text", default: "lj", required: true, section: "simulation" },
        { key: "atomStyle", label: "Atom style", type: "text", default: "atomic", required: true, section: "molecule" },
        { key: "pairStyle", label: "Pair style", type: "text", default: "lj/cut 12.0", required: true, section: "potential" },
        { key: "pairCoeff", label: "Pair coeff", type: "text", default: "1 1 1.0 1.0 12.0", required: true, section: "potential" },
        { key: "thermo", label: "Thermo interval", type: "integer", default: 100, min: 1, step: 10, required: true, validation: { min: 1 }, section: "output" },
        { key: "runSteps", label: "Run steps", type: "integer", default: 5000, min: 0, step: 100, required: true, validation: { min: 0 }, section: "protocol" },
        { key: "timestep", label: "Timestep", type: "number", default: 0.005, min: 0.0001, step: 0.001, required: true, validation: { positive: true }, section: "protocol" },
        { key: "temperature", label: "Temperature", type: "number", default: 1.0, min: 0.001, step: 0.1, required: true, validation: { positive: true }, section: "protocol" },
        { key: "density", label: "Density", type: "number", default: 0.75, min: 0.001, step: 0.05, required: false, validation: { positive: true }, section: "simulation" },
        { key: "beadCount", label: "Total CG bead count", type: "integer", default: 32, min: 1, step: 1, required: true, validation: { min: 1 }, section: "molecule" },
        { key: "chainCount", label: "Chain count", type: "integer", default: 4, min: 1, step: 1, required: true, validation: { min: 1 }, section: "molecule" },
        { key: "repeatCountPerChain", label: "Repeat units per chain", type: "integer", default: 8, min: 1, step: 1, required: true, validation: { min: 1 }, section: "molecule" },
        { key: "deformAxis", label: "Deformation axis", type: "select", default: "none", options: ["none", "x", "y", "z"], required: false, section: "protocol" },
        { key: "deformRate", label: "Deformation rate", type: "number", default: 0.0, step: 0.0001, required: false, section: "protocol" }
      ],
