(function initLammpsCaseCore(root, factory) {
  ...
  function generatePackmolInput(caseDefinition) {
    const current = normalizeCase(caseDefinition);
    if (current.caseType !== "cg_scaffold") {
      return [
        "# PACKMOL input is not required for this public single-box demo.",
        "# Save and run in.lammps directly in an approved local LAMMPS environment."
      ].join("\n") + "\n";
    }
    const beadCount = Math.max(1, Math.floor(toNumber(current.beadCount, 32)));
    const density = Math.max(0.001, toNumber(current.density, 0.75));
    const boxLength = Math.max(8, Math.cbrt(beadCount / density));
    return [
      "# PACKMOL input draft generated by LAMMPS Workbench Web Safe Mode",
      "# This web app does not run PACKMOL. Save this file and run PACKMOL manually if needed.",
      "tolerance 2.0",
      "filetype pdb",
      "output packed.pdb",
      "seed 12345",
      "",
      "structure cg_bead.pdb",
      `  number ${beadCount}`,
      `  inside box 0. 0. 0. ${formatNumber(boxLength)} ${formatNumber(boxLength)} ${formatNumber(boxLength)}`,
      "end structure",
      "",
      "# Required companion placeholder structure file:",
      "# cg_bead.pdb should contain one representative CG bead for PACKMOL placement."
    ].join("\n") + "\n";
  }

  function generateMoltemplateLt(caseDefinition) {
    const current = normalizeCase(caseDefinition);
    if (current.caseType !== "cg_scaffold") {
      return [
        "# Moltemplate LT draft is not required for this public single-box demo.",
        "# Save and run in.lammps directly in an approved local LAMMPS environment."
      ].join("\n") + "\n";
    }
    const beadCount = Math.max(1, Math.floor(toNumber(current.beadCount, 32)));
    const chainCount = Math.max(1, Math.floor(toNumber(current.chainCount, 4)));
    const repeatCountPerChain = Math.max(1, Math.floor(toNumber(current.repeatCountPerChain, 8)));
    const density = Math.max(0.001, toNumber(current.density, 0.75));
    const boxLength = Math.max(8, Math.cbrt(beadCount / density));
    return [
      "# Moltemplate LT draft generated by LAMMPS Workbench Web Safe Mode",
      "# This web app does not run Moltemplate. Save this file and run Moltemplate manually if needed.",
      "",
      "CGBead {",
      ...
      "  }",
      "  write(\"Data Atoms\") {",
      "    $atom:b1 $mol:. @atom:CG 0.0 0.0 0.0 0.0",
      "  }",
      "}",
      "",
      `beads = new CGBead[${beadCount}]`,
      "",
      "write_once(\"Data Boundary\") {",
      `  0 ${formatNumber(boxLength)} xlo xhi`,
      `  0 ${formatNumber(boxLength)} ylo yhi`,
      `  0 ${formatNumber(boxLength)} zlo zhi`,
      "}",
      "",
      "write_once(\"In Init\") {",
      `  units ${current.units}`,
      ...
      `  pair_style ${current.pairStyle}`,
      `  pair_coeff @atom:CG @atom:CG ${String(current.pairCoeff).replace(/^(\S+\s+\S+\s+)?/, "")}`,
      "}",
      "",
      "write_once(\"In Run\") {",
      `  # chains=${chainCount}, repeat_units_per_chain=${repeatCountPerChain}`,
      `  velocity all create ${formatNumber(current.temperature)} 12345 mom yes rot no dist gaussian`,
      `  fix 1 all nvt temp ${formatNumber(current.temperature)} ${formatNumber(current.temperature)} 1.0`,
      `  timestep ${formatNumber(current.timestep)}`,
      `  thermo ${current.thermo}`,
      "  thermo_style custom step temp press etotal pe ke density",
      `  dump CG all custom ${current.thermo} cg_scaffold.dump id type x y z`,
      `  run ${Math.floor(toNumber(current.runSteps, 5000))}`,
      "}"
    ].join("\n") + "\n";
  }
  ...
  return {
    ...
    generateLammpsInput,
    generateCaseJson,
    generatePackmolInput,
    generateMoltemplateLt,
    generateProcedure
  };
});
